...
class CellAnalyzer:
    ...
    def _analyze_cell(self, cell: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                ...
                for node in ast.walk(stmt):
                    if isinstance(node, ast.Assign):
                        if isinstance(node.value, ast.Constant) and isinstance(
                            ...
                            if (
                                "/" in node.value.value
                                or ".csv" in node.value.value
                                or ".pkl" in node.value.value
                            ):
                                analysis["has_hardcoded_paths"] = True
                                analysis["hardcoded_values"].append(
                                    {"type": "path", "value": node.value.value}
                                )
            ...
            analysis["external_dependencies"] = (
                used_before_defined - builtins - local_scope_vars
            )
            ...
            for j in range(i - 1, -1, -1):  # Search backwards from current cell
                ...
                if common:
                    analysis["depends_on"].append(
                        {"cell_index": other["index"], "variables": list(common)}
                    )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        # Hardcoded paths
        cells_with_paths = [
            a for a in self.analysis_results if a["has_hardcoded_paths"]
        ]
        if cells_with_paths:
            issues.append(
                {
        ...
        return {
            ...
            "hardcoded_values_count": sum(
                len(a["hardcoded_values"]) for a in self.analysis_results
            ),
            "issues": issues,
        }
