...
class ProjectGenerator:
    ...
    def _generate_src_init(
        self, grouped_functions: Dict[str, List[Dict[str, Any]]]
    ) -> None:
    ...
    def _get_imports_for_functions(self, functions: List[Dict[str, Any]]) -> List[str]:
        ...
        if any(
            "np.ndarray" in str(func) or "np." in str(func.get("full_code", ""))
            for func in functions
        ):
        ...
        has_typing = any(
            "Tuple" in func.get("signature", "")
            or "List" in func.get("signature", "")
            or "Dict" in func.get("signature", "")
            for func in functions
        )
        if has_typing:
    ...
    def _generate_config(self) -> None:
        ...
        config = {
            ...
            "parameters": {"note": "Add configuration parameters here as needed"},
        }
