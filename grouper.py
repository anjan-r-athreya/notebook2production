...
class CellGrouper:
    ...
    def _detect_educational_notebook(self) -> bool:
        ...
        cells_with_deps = sum(
            1 for a in self.analysis_results if a.get("depends_on", [])
        )
    ...
    def _categorize_cell(self, analysis: Dict[str, Any]) -> str:
        ...
        if any(
            pattern in vars_str for pattern in ["df", "data", "dataset", "load", "read"]
        ):
            return "data"
        elif any(
            pattern in vars_str
            for pattern in ["x_", "y_", "feature", "scaled", "transform"]
        ):
            return "feature"
        elif any(
            pattern in vars_str for pattern in ["model", "train", "fit", "predict"]
        ):
            return "model"
    ...
    def _merge_related_groups(
        ...
        for next_group in groups[1:]:
            ...
            if should_merge:
                ...
                current_group["variables_defined"].update(
                    next_group["variables_defined"]
                )
                current_group["external_dependencies"].update(
                    next_group["external_dependencies"]
                )
                # Keep the most specific category
                if (
                    next_group["category"] != "utility"
                    and current_group["category"] == "utility"
                ):
    ...
    def _is_quality_function(self, group_info: Dict[str, Any]) -> bool:
        ...
        for cell_idx in group_info["cells"]:
            cell_analysis = next(
                (a for a in self.analysis_results if a["index"] == cell_idx), None
            )
        ...
        if missing_deps:
            ...
            if not group_info["parameters"] and not group_info["returns"]:
                ...
                for cell_idx in group_info["cells"]:
                    cell_analysis = next(
                        (a for a in self.analysis_results if a["index"] == cell_idx),
                        None,
                    )
    ...
    def _should_merge(self, group1: Dict[str, Any], group2: Dict[str, Any]) -> bool:
        ...
        if self._has_execution_order_issues(group1) or self._has_execution_order_issues(
            group2
        ):
            return False
        ...
        if (
            group1["category"] == "utility" or group2["category"] == "utility"
        ) and gap <= 2:
            return True
    ...
    def _analyze_group(self, group: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "suggested_name": self._suggest_function_name(
                group["category"], parameters, returns
            ),
        }

    def _suggest_function_name(
        self, category: str, parameters: Set[str], returns: Set[str]
    ) -> str:
